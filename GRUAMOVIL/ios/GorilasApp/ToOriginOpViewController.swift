//
//  ToOriginOpViewController.swift
//  GorilasApp
//
...
class ToOriginOpViewController: UIViewController, UITableViewDelegate, UITableViewDataSource , CLLocationManagerDelegate, GMSMapViewDelegate  {
    ...
    override func viewDidLoad() {
        ...
        self.Timer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(ToOriginOpViewController.drawOriginMarker), userInfo: nil, repeats: false);
        RunLoop.main.add(self.Timer, forMode: RunLoopMode.commonModes);

        if (self.Order.origin.address != "Sin direccion" && self.Order.origin.address != "") {
            self.DrawMarker(self.Order.origin.lat, Long: self.Order.origin.long, title: "Destino", color: UIColor.red);
        }
    ...
    func drawOriginMarker() {
        ...
        self.Timer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(ToOriginOpViewController.FocusMapOnAllMarkers), userInfo: nil, repeats: false);
        RunLoop.main.add(self.Timer, forMode: RunLoopMode.commonModes);
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ToDestinySegue" {
            if let dest = segue.destination as? ToDestinyOpViewController {
                ...
                dest.isSchedule = self.isSchedule;
            }
        }
    }
    ...
    func addOverlayToMapView(){
        let directionURL = "https://maps.googleapis.com/maps/api/directions/json?origin=\(self.ActualLocation.lat),\(self.ActualLocation.long)&destination=\(self.Order.origin.lat),\(self.Order.origin.long)&mode=driving&key=\(VARS().getGoogleKey())";
