//
//  NewOrderOpViewController.swift
//  GorilasApp
//
...
class NewOrderOpViewController: UIViewController, UITableViewDelegate, UITableViewDataSource , CLLocationManagerDelegate, GMSMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.Timer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(NewOrderOpViewController.addOverlayOrigin), userInfo: nil, repeats: false);
        RunLoop.main.add(self.Timer, forMode: RunLoopMode.commonModes);
        
        if (self.Order.isQuotation) {
            self.AcceptButton.setTitle("ACEPTAR COTIZACIÓN", for: .normal);
            self.TextDescription.text = "DESCRIPCION DE COTIZACIÓN";
            self.TextDescription.textColor = UIColor.white;
            self.ViewDescription.backgroundColor = UIColor(hexString: "#D93829");
        }else{
            self.AcceptButton.setTitle("ACEPTAR ORDEN", for: .normal);
            self.TextDescription.text = "DESCRIPCION DE ORDEN";
        }
    }
    func reloadData() {
    ...
    @IBAction func AcceptOrder(_ sender: AnyObject) {
        if (self.isClickedConfirm == false) {
            self.isClickedConfirm = true;
            SocketIOManager.sharedInstance.SendState(self.Order._id, user_id: self.UsuarioEnSesion._id, state: "AcceptOrder");
        }
    }
    
    @IBAction func CancelOrder(_ sender: AnyObject) {
        if (self.isClickedCancel == false) {
    ...
    func addOverlayOrigin() {
        let locToUsOrigin = "https://maps.googleapis.com/maps/api/directions/json?origin=\(self.ActualLocation.lat),\(self.ActualLocation.long)&destination=\(self.Order.origin.lat),\(self.Order.origin.long)&mode=driving&key=\(VARS().getGoogleKey())";
        
        Alamofire.request(locToUsOrigin).responseJSON { response in
                ...
                if (errornum == true){
                    ...
                    if routes != nil{
                        if (routes?.count)! > 0 {
                            ...
                            self.OrderDetailArray.append(["title": "HACIA ORIGEN DE USUARIO", "value": "\(Distance)   |   \(Time)","icon":"Home.png"]);
                            
                            self.MainTableView.reloadData();
                            
                            let overViewPolyLine = routes![0]["overview_polyline"]["points"].string
                            if overViewPolyLine != nil{
                                self.addPolyLineWithEncodedStringInMap(overViewPolyLine!, color: UIColor.red)
                            }
                        }
                    }
                }
